...
def coords_grid_one(b, h, w, device=None):
    z = torch.meshgrid(torch.arange(h), torch.arange(w))[0]  # [H, W]

    grid = z.float()  # [1, H, W]
    grid = grid[None].repeat(b, 1, 1, 1)  # [B, 1, H, W]
...
def disp_warp(feature, disp, mask=False, padding_mode='zeros'):
    b, c, h, w = feature.size()
    assert disp.size(1) == 2

    grid = coords_grid(b, h, w).to(disp.device) + disp  # [B, 2, H, W]
