...
from dataloader.datasets import (FlyingThings3D, KITTI15, KITTI12,
...
@torch.no_grad()
def create_sceneflow_submission(model,
                            dataset='things', 
                            output_path='output',
                            padding_factor=16,
    ...
    test_dataset = FlyingThings3D(mode='TEST', transform=val_transform)

    # TestImgLoader = DataLoader(val_dataset, batch_size, shuffle=False, num_workers=2, drop_last=False)
    ...
    for i, sample in enumerate(test_dataset):
        ...
        # im_color = cv2.applyColorMap(cv2.convertScaleAbs(pred_disp, alpha=1), cv2.COLORMAP_JET)
        
        uint16_disp = (pred_disp.cpu().numpy() * 256.).astype(np.uint16)

        uint16_disp -= uint16_disp.min()
        uint16_disp = uint16_disp / (uint16_disp.max() - uint16_disp.min())
        uint16_disp *= 255
        uint16_disp = 255 - uint16_disp
        im_color = cv2.applyColorMap(cv2.convertScaleAbs(uint16_disp,alpha=1),cv2.COLORMAP_JET)
        im=Image.fromarray(im_color)
        
        skimage.io.imsave(save_name, im_color)
...
def create_middlebury_submission(model,
    ...
    for i, sample in enumerate(test_dataset):
        ...
        if save_vis_disp:
            ...
            save_disp_name = os.path.join(save_disp_dir, 'disp0DFGANet.pfm')
            # save disp
            write_pfm(save_disp_name, pred_disp.cpu().numpy())
            # save runtime
            save_runtime_name = os.path.join(save_disp_dir, 'timeDFGANet.txt')
            with open(save_runtime_name, 'w') as f:
...
def validate_things(model,
    ...
    for i, sample in enumerate(TestImgLoader):
        if i == len(TestImgLoader):
            break
        if i % 100 == 0:
            print('=> Validating %d/%d' % (i, len(TestImgLoader)))

        left = sample['left'].to(device)  # [b, 3, H, W]
        right = sample['right'].to(device)  # [b, 3, H, W]
        ...
        # remove padding
        if inference_size is None:
            pred_disp = padder.unpad(pred_disp)  # [H, W]
        else:
            # resize back
            pred_disp = F.interpolate(pred_disp.unsqueeze(1), size=ori_size, mode='bilinear',
                                      align_corners=True).squeeze(1)  # [H, W]
            pred_disp = pred_disp * ori_size[-1] / float(inference_size[-1])
    ...
    mean_epe = val_epe / len(TestImgLoader)
    mean_d1 = val_d1 / len(TestImgLoader)

    print('Validation things EPE: %.5f, D1: %.4f' % (
        mean_epe, mean_d1))
...
def validate_kitti15(model,
    ...
    print('{%d}Validation KITTI15 EPE: %.3f, D1: %.4f, 3px: %.4f' % (i,
