...
#     def __init__(self, output_dim=128):
#         super(CNNEncoder, self).__init__()
#         self.feat = Feature(output_dim)
#         self.con = Context(output_dim)
#
#     def forward(self, x):
#         fea = self.feat(x)
#         con = self.con(x)
...
if __name__ == "__main__":
    model = CNNEncoder(output_dim=128, norm_layer=nn.InstanceNorm2d)
    # 查看那些层被冻结了
    for k, v in model.named_parameters():
        print('{}: {}'.format(k, v.requires_grad))
    x = torch.Tensor(torch.randn(1, 3, 288, 512))
    y = torch.Tensor(torch.randn(1, 3, 288, 512))
    z = torch.cat((x, y), 1)
    start_time = time.time()
    out = model(x)  # 0.1775 s
    end_time = time.time()
