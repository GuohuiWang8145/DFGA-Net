...
def upsample_disp_with_mask(disp, up_mask, upsample_factor,
                            is_depth=False):
    # convex upsampling following raft

    mask = up_mask
    b, disp_channel, h, w = disp.shape
    mask = mask.view(b, 1, 9, upsample_factor, upsample_factor, h, w)  # [B, 1, 9, K, K, H, W]
    mask = torch.softmax(mask, dim=2)

    multiplier = 1 if is_depth else upsample_factor
    up_disp = F.unfold(multiplier * disp, [3, 3], padding=1)
    up_disp = up_disp.view(b, disp_channel, 9, 1, 1, h, w)  # [B, 2, 9, 1, 1, H, W]

    up_disp = torch.sum(mask * up_disp, dim=2)  # [B, 2, K, K, H, W]
    up_disp = up_disp.permute(0, 1, 4, 2, 5, 3)  # [B, 2, K, H, K, W]
    up_disp = up_disp.reshape(b, disp_channel, upsample_factor * h,
